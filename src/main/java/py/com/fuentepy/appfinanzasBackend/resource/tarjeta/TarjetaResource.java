...
public class TarjetaResource {
    ...
    @Secured({"ROLE_ADMIN"})
    @PutMapping(value = "/deuda", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> deuda(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                   @Valid @RequestBody TarjetaRequestDeuda tarjetaRequestDeuda) {
        HttpStatus httpStatus;
        BaseResponse response;
        MessageResponse message;
        List<MessageResponse> messages = new ArrayList<>();
        Long usuarioId = userPrincipal.getId();
        Long id = tarjetaRequestDeuda.getId();
        if (tarjetaService.findByIdAndUsuarioId(id, usuarioId) == null) {
            httpStatus = HttpStatus.NOT_FOUND;
            ...
            try {
                if (tarjetaService.deuda(tarjetaRequestDeuda, usuarioId)) {
                    httpStatus = HttpStatus.CREATED;
                    message = new MessageResponse(StatusLevel.INFO, "Deuda de Tarjeta ha sido cargada con éxito!");
                    messages.add(message);
                    response = new BaseResponse(httpStatus.value(), messages);
                } else {
                    httpStatus = HttpStatus.INTERNAL_SERVER_ERROR;
                    message = new MessageResponse(StatusLevel.ERROR, "Deuda no se pudo cargar!");
                    messages.add(message);
                    response = new BaseResponse(httpStatus.value(), messages);
                }
            } catch (DataAccessException e) {
    ...
    public ResponseEntity<?> pagar(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
        ...
        if (tarjetaService.findByIdAndUsuarioId(id, usuarioId) == null) {
            ...
            try {
                Movimiento movimiento = tarjetaService.pagar(tarjetaRequestPago, usuarioId);
                if (movimiento != null) {
                    httpStatus = HttpStatus.CREATED;
