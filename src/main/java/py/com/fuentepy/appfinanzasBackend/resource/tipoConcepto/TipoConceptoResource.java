...
public class TipoConceptoResource {
    ...
    public ResponseEntity<?> index(@ApiIgnore Pageable pageable) {
        Page<TipoConceptoModel> tipoConceptos = null;
    ...
    public ResponseEntity<?> show(@PathVariable Integer id) {
        ...
        try {
            TipoConceptoModel tipoConceptoModel = tipoConceptoService.findById(id);
            if (tipoConceptoModel == null) {
                httpStatus = HttpStatus.NOT_FOUND;
                message = new MessageResponse(StatusLevel.WARNING, "Error: El Tipo Concepto Nro: ".concat(id.toString()).concat(" no existe en la base de datos!"));
                ...
                messages.add(message);
                response = new TipoConceptoResponse(httpStatus.value(), messages, tipoConceptoModel);
            }
        } catch (DataAccessException e) {
    ...
    public ResponseEntity<?> create(@Valid @RequestBody TipoConceptoRequestNew tipoConceptoRequestNew,
        ...
        if (result.hasErrors()) {
            ...
            try {
                if (tipoConceptoService.create(tipoConceptoRequestNew)) {
                    httpStatus = HttpStatus.CREATED;
                    message = new MessageResponse(StatusLevel.INFO, "El Tipo Concepto ha sido creada con éxito!");
                    messages.add(message);
                    response = new BaseResponse(httpStatus.value(), messages);
                } else {
                    httpStatus = HttpStatus.INTERNAL_SERVER_ERROR;
                    message = new MessageResponse(StatusLevel.ERROR, "El Tipo Concepto no se pudo crear!");
                    messages.add(message);
                    response = new BaseResponse(httpStatus.value(), messages);
                }
            } catch (DataAccessException e) {
    ...
    public ResponseEntity<?> update(@Valid @RequestBody TipoConceptoRequestUpdate tipoConceptoRequestUpdate, BindingResult result) {
        HttpStatus httpStatus;
        BaseResponse response;
        MessageResponse message;
        List<MessageResponse> messages = new ArrayList<>();
        Integer id = tipoConceptoRequestUpdate.getId();
        if (result.hasErrors()) {
            httpStatus = HttpStatus.BAD_REQUEST;
            for (FieldError err : result.getFieldErrors()) {
                message = new MessageResponse(StatusLevel.INFO, "El campo '".concat(err.getField()).concat("' ").concat(err.getDefaultMessage()));
            ...
            if (tipoConceptoService.findById(id) == null) {
                httpStatus = HttpStatus.NOT_FOUND;
                message = new MessageResponse(StatusLevel.WARNING, "Error: no se pudo editar, El Tipo Concepto Ahorro Nro: ".concat(id.toString()).concat(" no existe en la base de datos!"));
                messages.add(message);
                response = new BaseResponse(httpStatus.value(), messages);
            } else {
                try {
                    if (tipoConceptoService.update(tipoConceptoRequestUpdate)) {
                        httpStatus = HttpStatus.CREATED;
                        message = new MessageResponse(StatusLevel.INFO, "El Tipo Concepto ha sido actualizada con éxito!");
                        messages.add(message);
                        response = new BaseResponse(httpStatus.value(), messages);
                    } else {
                        httpStatus = HttpStatus.INTERNAL_SERVER_ERROR;
                        message = new MessageResponse(StatusLevel.ERROR, "La TipoConcepto no se pudo actualizar!");
                        messages.add(message);
                        response = new BaseResponse(httpStatus.value(), messages);
                    }
                } catch (DataAccessException e) {
